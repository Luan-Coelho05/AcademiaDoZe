...
public class ColaboradorRepository : BaseRepository, IColaboradorRepository
{
        ...
        try
        {
            ...
            if (result.IsFailure)
            {
                throw new InfrastructureException(
                    ...
                    $"Erro de domínio ao mapear colaborador ID {id}: {string.Join(", ",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "UPDATE tb_colaborador SET cpf = @Cpf, nome = @Nome, nascimento = @Nascimento, telefone = @Telefone, email = @Email, logradouro_id = @LogradouroId, numero = @Numero, complemento = @Complemento, senha = @Senha, foto = @Foto, admissao = @Admissao, tipo = @Tipo, vinculo = @Vinculo WHERE id_colaborador = @Id";
