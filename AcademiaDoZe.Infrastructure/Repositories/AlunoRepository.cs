...
public class AlunoRepository : BaseRepository, IAlunoRepository
{
        ...
        try
        {
            ...
            if (result.IsFailure)
            {
                throw new InfrastructureException(
                    ...
                    $"Erro de domínio ao mapear aluno ID {id}: {string.Join(", ",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "UPDATE tb_aluno SET cpf = @Cpf, nome = @Nome, nascimento = @Nascimento, telefone = @Telefone, email = @Email, logradouro_id = @LogradouroId, numero = @Numero, complemento = @Complemento, senha = @Senha, foto = @Foto WHERE id_aluno = @Id";
