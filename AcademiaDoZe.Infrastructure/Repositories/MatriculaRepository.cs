...
public class MatriculaRepository : BaseRepository, IMatriculaRepository
{
        ...
        try
        {
            ...
            if (result.IsFailure)
            {
                throw new InfrastructureException(
                    ...
                    $"Erro de domínio ao mapear matrícula ID {id}: {string.Join(", ",
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            string query = FormatInsertQuery(
                "INSERT INTO tb_matricula (aluno_id, plano, data_inicio, data_fim, objetivo, restricao_medica, obs_restricao, laudo_medico) VALUES (@AlunoId, @Plano, @DataInicio, @DataFim, @Objetivo, @RestricaoMedica, @ObsRestricao, @LaudoMedico)");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "UPDATE tb_matricula SET aluno_id = @AlunoId, plano = @Plano, data_inicio = @DataInicio, data_fim = @DataFim, objetivo = @Objetivo, restricao_medica = @RestricaoMedica, obs_restricao = @ObsRestricao, laudo_medico = @LaudoMedico WHERE id_matricula = @Id";
