...
{
    // Troque este valor junto com o SelectedDatabaseType do TestBase, a cada rodada:
    // "SQLite", "MySQL" ou "SQLServer"
    private const string NomeCidadeSgbd = "SQLServer";
        ...
        LogradouroRepository logradouroRepo)
    {
        ...
        {
            throw new Exception(
                $"Falha ao criar Logradouro: {string.Join(", ",
                logradouroResult.Notifications.Select(n => n.Mensagem))}");
        }
    ...
    public async Task Logradouro_Adicionar_E_ObterPorId_Sucesso()
    {
        ...
        Assert.Equal("Luan", inserido.Nome);
    ...
    public async Task Logradouro_ObterPorCidade_FiltragemCorreta()
    {
        ...
        var resultados =
            await _repository.ObterPorCidade(
                cidadeUnica); // <-- linha alterada, sem o .ToLower()
