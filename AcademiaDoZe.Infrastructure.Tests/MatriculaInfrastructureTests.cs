...
public class MatriculaInfrastructureTests : TestBase
{
        ...
        Arquivo? laudo = null)
    {
        ...
        {
            throw new Exception(
                $"Falha ao criar Matricula no Helper: {string.Join(", ",
                matriculaResult.Notifications.Select(n => n.Mensagem))}");
        }
    ...
    public async Task Matricula_Adicionar_E_ObterPorId_Sucesso()
    {
        ...
        var inserida = await CriarEInserirMatriculaAsync(
            ...
            obsRestricao: $"Usar medicação ao acordar - {SiglaSgbd}",
            laudo: laudo
        );
        ...
        Assert.Equal($"Usar medicação ao acordar - {SiglaSgbd}", obtida.ObservacoesRestricoes);
    ...
    public async Task Matricula_RestricoesMedicas_ComVariacoesMultiplaEscolha_PersisteEObtemCorretamente()
    {
        ...
        var matricula = await CriarEInserirMatriculaAsync(
            ...
            obsRestricao: $"Evitar exercícios de alto impacto e hipertensão - {SiglaSgbd}",
            laudo: laudo
        );
        ...
        Assert.Equal($"Evitar exercícios de alto impacto e hipertensão - {SiglaSgbd}", obtida.ObservacoesRestricoes);
        Assert.NotNull(obtida.LaudoMedico);
        Assert.Equal(laudo.Conteudo, obtida.LaudoMedico.Conteudo);
    }
    ...
    public async Task Matricula_Atualizar_Sucesso()
    {
        ...
        var inserida = await CriarEInserirMatriculaAsync(aluno, MatriculaPlano.Mensal, restricoes: MatriculaRestricoes.Alergias, obsRestricao: $"Alergia leve - {SiglaSgbd}");
        ...
        var matriculaAtualizada = Matricula.Criar(
            ...
            observacoesRestricoes: $"Restrição médica atualizada com novas opções - {SiglaSgbd}"
        ).Value!;
        ...
        Assert.Equal("Luan Coelho", resultado.Objetivo);
        ...
        Assert.Equal("Luan Coelho", noBanco.Objetivo);
