...
public class AlunoInfrastructureTests : TestBase
{
        ...
        LogradouroRepository logradouroRepo)
    {
        ...
        {
            throw new Exception(
                $"Falha ao criar Aluno: {string.Join(", ",
                alunoResult.Notifications.Select(n => n.Mensagem))}");
        }
    ...
    public async Task Aluno_ObterPorNome_FiltragemCorreta()
    {
        ...
        var resultados = await _alunoRepo.ObterPorNome("Luan");

        Assert.NotNull(resultados);
        Assert.Contains(resultados, a => a.Id == aluno.Id);

        var resultadosVazio = await _alunoRepo.ObterPorNome("NomeQueNaoExiste123");
    ...
    public async Task Aluno_TrocarSenha_SucessoEFalha()
    {
        ...
        var novaSenha = Senha.Criar($"Nova{SiglaSgbd}Senha1").Value!;
        ...
        Assert.Equal(novaSenha.Valor, atualizado.Senha.Valor);
